...
void main()
{
	...
	float s = max(normalize(halfAngle).z, 0);
	
	gl_FragColor = 0.50*LMa + 0.50*(d * LMd) + 0.50*pow(s,shininess)*LMs + 0.50*texture2D(decal,vec2(normalMapTexCoord.x * 6, normalMapTexCoord.y * 2));  // XXX fix me
