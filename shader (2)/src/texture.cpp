...
{
	float current = 	image[(width * j) + i];
	float right = 		image[(width * j) + ((i + 1) % width)];
	//float left = 		image[(width * j) + ((i - 1) % width)];
	float top =			image[(width * ((j-1) % height)) + i];
	//float bottom =		image[(width * ((j+1) % height)) + i];
	
	float3 normal = normalize(float3((right - current)*scale / (width), (top - current)*scale / (height), 1));  // XXX fix me
